...
class MainActivity : AppCompatActivity() {
    ...
    private val SELECT          = LemonState.SELECT.value
    // SQUEEZE represents the "squeeze lemon" state
    private val SQUEEZE         = LemonState.SQUEEZE.value
    // DRINK represents the "drink lemonade" state
    private val DRINK           = LemonState.DRINK.value
    // RESTART represents the state where the lemonade has be drunk and the glass is empty
    private val RESTART         = LemonState.RESTART.value
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState != null) {
            lemonadeState   = savedInstanceState.getString(LEMONADE_STATE, LemonState.SELECT.value)
    ...
    private fun clickLemonImage() {
        ...
        when (lemonadeState) {
            ...
            LemonState.SELECT.value -> {
                lemonadeState   = LemonState.SQUEEZE.value
            ...
            LemonState.SQUEEZE.value -> {
                squeezeCount    += 1
                lemonSize       -= 1

                if (lemonSize == 0) {
                    lemonadeState   = LemonState.DRINK.value
                    lemonSize = -1
                }
            }

            // TODO: When the image is clicked in the DRINK state the state should become RESTART
            LemonState.DRINK.value -> {
                lemonadeState   = LemonState.RESTART.value
                lemonSize       = -1
            }

            // TODO: When the image is clicked in the RESTART state the state should become SELECT
            LemonState.RESTART.value -> lemonadeState = LemonState.SELECT.value
        }
    ...
    private fun showSnackbar(): Boolean {
        if (lemonadeState != LemonState.SQUEEZE.value) {
            return false
        }
